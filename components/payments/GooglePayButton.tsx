 ...
 * Verwendet Stripe Payment Request Button für Google Pay
 */
'use client'
...
export default function GooglePayButton({
  ...
  const initializePaymentRequest = useCallback(async () => {
    ...
    try {
      ...
      if (canMakePayment && canMakePayment.googlePay) {
        ...
        buttonRef.current.innerHTML = '<p class="text-sm text-gray-500">Google Pay nicht verfügbar</p>'
      }
    } catch (error) {
  ...
  return (
    <div className="w-full">
      ...
      {disabled && (
        <p className="mt-2 text-xs text-gray-500">Google Pay ist deaktiviert</p>
      )}
    </div>
  )
}
